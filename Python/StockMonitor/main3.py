...
def sendEmail():
    ...
    # 发送的主题
    subject = '{0}-{1}-{2} 报表'.format(localtime.tm_year, str(localtime.tm_mon).zfill(2),
                                       str(localtime.tm_mday).zfill(2))
    html = getHtml()

    message = MIMEText(html, 'html', 'utf-8')
    message['Subject'] = Header(subject, 'utf-8')
    message['From'] = formataddr(['股票助手', sender])
    message['To'] = ",".join(receivers)

    smtpObj = smtplib.SMTP()
    smtpObj.connect(mail_host, str(mail_port))  # 25 为 SMTP 端口号  outlook端口587  网易是25

    smtpObj.starttls()  # 微软的 outlook 是此种加密方式  STARTTLS
...
def main():
    while True:
        ...
        if localtime.tm_hour == 9 and localtime.tm_min > 0:
            sendEmail()
            break
        else:
